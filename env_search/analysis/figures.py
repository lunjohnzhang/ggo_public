...
def collect(manifest: str,
    ...
    metric_names = {
        ...
        # "Total Num of Restarts": "Total Num of Restarts",
        # "Num Restarts from Area": "Num Restarts from Area",
    }
    ...
    for env in figure_data:
        ...
        with mpl_style_file("comparison.mplstyle") as f, plt.style.context(f):
            ...
            for (row_val, col_val), ax in grid.axes_dict.items():
                ...
                if col_val == "Best Objective":
                    baseline_val = None
                    if "Kiva_Human_100_agents_QD-score" in row_val and "directed" not in row_val:
                        baseline_val = 3.42
                    elif "Kiva_Human_200_agents_QD-score" in row_val:
                        baseline_val = 6.36
                    elif "Kiva_Human_17x16_60_agents" in row_val:
                        baseline_val = 4.39
                    elif "Kiva_Human_5x5_5_agents" in row_val:
                        baseline_val = 1.322
                    elif "Comp_Random_100" in row_val:
                        baseline_val = 1.053
                    elif "Comp_Random_200" in row_val:
                        baseline_val = 1.753
                    elif "Comp_Random_400" in row_val:
                        baseline_val = 2.234
                    elif "Comp_Random_600" in row_val:
                        baseline_val = 2.064
                    elif "Comp_Random_800" in row_val:
                        baseline_val = 1.616
                ...
                if "Kiva" in env:
                    ...
                    if col_val == "QD-score":
                        if env == "Kiva_w_mode_small":
                            ...
                            # ax.set_ylim(0, 20000)
                    elif col_val == "Best Objective":
                        if "Kiva_Human_100_agents" in env:
                            ax.set_yticks([2.5, 3, 3.5])
                            ax.set_yticklabels([2.5, 3, 3.5])
                            ax.set_ylim([2.5, 3.5])
                        elif "Kiva_Human_5x5_5_agents" in env:
                            ax.set_yticks([1.2, 1.3, 1.4])
                            ax.set_yticklabels([1.2, 1.3, 1.4])
                            ax.set_ylim([1.2, 1.4])
            ...
            grid.fig.legend(
                *legend_info(all_algos, palette, markers),
                bbox_to_anchor=[0.5, 1.2] if len(all_algos) >= 6 else [0.5, 1.1],
                loc="upper center",

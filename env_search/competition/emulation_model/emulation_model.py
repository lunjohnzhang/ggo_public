"""Provides primary CompetitionEmulationModel."""
import logging
...
from env_search.competition.emulation_model.aug_buffer import AugBuffer
from env_search.competition.emulation_model.double_aug_buffer import DoubleAugBuffer
from env_search.competition.emulation_model.buffer import Buffer, Experience
from env_search.competition.emulation_model.networks import (
    CompetitionConvolutional, CompetitionAugResnetOccupancy,
...
NETWORKS = (CompetitionConvolutional, )


@gin.configurable(denylist=["seed"])
class CompetitionEmulationModel:
    """Class for competition emulation model.
    ...
    Usage:
        model = CompetitionEmulationModel(...)
    ...
    def __init__(
        ...
        seed: int = None,
        obj_measure_loss_weight: List[float] = [1, 1, 1],
    ):
        ...
        if pre_network_type is None:
            ...
            self.pre_network = pre_network_type().load_from_saved_weights().to(
                DEVICE)
            if isinstance(self.pre_network, CompetitionAugResnetOccupancy):
        ...
        self.obj_measure_loss_weight = obj_measure_loss_weight
    ...
    def train(self, end_to_end=False):
        ...
        # Train each part of the model separately
            ...
            if self.pre_network is not None:
                ...
                elif isinstance(self.pre_network, CompetitionAugResnetOccupancy):
                    freeze_params(self.network)
    ...
    def compute_obj_measure_loss(self, inputs, objs, measures, loss_calc):
        ...
        # Weights
        obj_loss_weight = self.obj_measure_loss_weight[0]
        measure_loss_weight = self.obj_measure_loss_weight[1:]
        assert len(measure_loss_weight) == measure_dim
        ...
        if self.prediction_type == "classification":
            ...
            obj_loss = loss_calc(pred_objs, obj_classes) * obj_loss_weight
            measure_losses = [
                loss_calc(pred_measures[i], measure_classes[i]) *
                measure_loss_weight[i] for i in range(measure_dim)
            ]
        else:
            obj_loss = loss_calc(pred_objs.flatten(), objs) * obj_loss_weight
            measure_losses = [
    ...
    def save(self, pickle_path: Path, pytorch_path: Path):
        ...
        """
        logger.info("Saving CompetitionEmulationModel pickle data")
        ...
        logger.info("Saving CompetitionEmulationModel PyTorch data")
        state_dict = {
